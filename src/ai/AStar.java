...
public class AStar {
	 ...
	 * @param int layer
	 */
	public AStar(GGSprite sprite, GGTileMap map, int layer) {
		this.sprite = sprite;
		this.map = map;
		this.layer = layer;
	}
...
//		neighbord[7] = new Node(node, node.x-1, node.y-1, node.traversed+1.4);
		...
		for(int i = 0; i < neighbord.length; i++){
			...
			/* Collision */
			int plusX = (int) (sprite.width/map.tileSizeX);
			int plusY = (int) (sprite.height/map.tileSizeY);
			boolean collid = false;
			for(int x=0; x<plusX; x++){
				for(int y=0; y<plusY; y++){
					if(map.layers.get(layer)[ntest.y+x][ntest.x+y]>0){
						collid = true;
						break;
					}
				}
